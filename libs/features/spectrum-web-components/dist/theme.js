...
import { supportsAdoptingStyleSheets as h } from "/libs/features/spectrum-web-components/dist/base.js";
var d = ["spectrum", "express"];
var m = ["medium", "large", "medium-express", "large-express"];
var c = ["light", "lightest", "dark", "darkest", "light-express", "lightest-express", "dark-express", "darkest-express"];
var a = class a2 extends HTMLElement {
  constructor() {
    super();
    this._dir = "";
    ...
    const e4 = document.importNode(a2.template.content, true);
    this.shadowRoot.appendChild(e4), this.shouldAdoptStyles(), this.addEventListener("sp-query-theme", this.onQueryTheme), this.addEventListener("sp-language-context", this._handleContextPresence), this.updateComplete = this.__createDeferredPromise();
  }
  ...
  set dir(e4) {
    if (e4 === this.dir)
      return;
    this.setAttribute("dir", e4), this._dir = e4;
    const t3 = e4 === "rtl" ? e4 : "ltr";
    this.trackedChildren.forEach((s) => {
      s.setAttribute("dir", t3);
    });
  }
  get dir() {
    return this._dir;
  }
  attributeChangedCallback(e4, t3, s) {
    t3 !== s && (e4 === "color" ? this.color = s : e4 === "scale" ? this.scale = s : e4 === "lang" && s ? (this.lang = s, this._provideContext()) : e4 === "theme" ? this.theme = s : e4 === "dir" && (this.dir = s));
  }
  requestUpdate() {
    window.ShadyCSS !== void 0 && !window.ShadyCSS.nativeShadow ? window.ShadyCSS.styleElement(this) : this.shouldAdoptStyles();
  }
  get theme() {
    const e4 = a2.themeFragmentsByKind.get("theme"), { name: t3 } = e4 && e4.get("default") || {};
    return this._theme || t3 || "";
  }
  set theme(e4) {
    if (e4 === this._theme)
      return;
    const t3 = e4 && d.includes(e4) ? e4 : this.theme;
    t3 !== this._theme && (this._theme = t3, this.requestUpdate()), t3 ? this.setAttribute("theme", t3) : this.removeAttribute("theme");
  }
  get color() {
    const e4 = a2.themeFragmentsByKind.get("color"), { name: t3 } = e4 && e4.get("default") || {};
    return this._color || t3 || "";
  }
  set color(e4) {
    if (e4 === this._color)
      return;
    const t3 = e4 && c.includes(e4) ? e4 : this.color;
    t3 !== this._color && (this._color = t3, this.requestUpdate()), t3 ? this.setAttribute("color", t3) : this.removeAttribute("color");
  }
  get scale() {
    const e4 = a2.themeFragmentsByKind.get("scale"), { name: t3 } = e4 && e4.get("default") || {};
    return this._scale || t3 || "";
  }
  set scale(e4) {
    if (e4 === this._scale)
      return;
    const t3 = e4 && m.includes(e4) ? e4 : this.scale;
    t3 !== this._scale && (this._scale = t3, this.requestUpdate()), t3 ? this.setAttribute("scale", t3) : this.removeAttribute("scale");
  }
  get styles() {
    const e4 = [...a2.themeFragmentsByKind.keys()], t3 = (r2, i, n) => {
      const o = n && n !== "theme" && this.theme === "express" ? r2.get(`${i}-express`) : r2.get(i), l = i === "spectrum" || !n || this.hasAttribute(n);
      if (o && l)
        return o.styles;
    };
    return [...e4.reduce((r2, i) => {
      const n = a2.themeFragmentsByKind.get(i);
      let o;
      if (i === "app" || i === "core")
        o = t3(n, i);
      else {
  ...
  onQueryTheme(e4) {
    ...
    e4.preventDefault();
    const { detail: t3 } = e4;
    t3.color = this.color || void 0, t3.scale = this.scale || void 0, t3.lang = this.lang || document.documentElement.lang || navigator.language, t3.theme = this.theme || void 0;
  }
  connectedCallback() {
    if (this.shouldAdoptStyles(), window.ShadyCSS !== void 0 && window.ShadyCSS.styleElement(this), a2.instances.add(this), !this.hasAttribute("dir")) {
      let e4 = this.assignedSlot || this.parentNode;
      for (; e4 !== document.documentElement && !(e4 instanceof a2); )
        e4 = e4.assignedSlot || e4.parentNode || e4.host;
      this.dir = e4.dir === "rtl" ? e4.dir : "ltr";
    }
  }
  disconnectedCallback() {
    a2.instances.delete(this);
  }
  startManagingContentDirection(e4) {
    this.trackedChildren.add(e4);
  }
  stopManagingContentDirection(e4) {
    this.trackedChildren.delete(e4);
  }
  ...
  adoptStyles() {
    ...
    if (window.ShadyCSS !== void 0 && !window.ShadyCSS.nativeShadow && window.ShadyCSS.ScopingShim) {
      const t3 = [];
      for (const [s, r2] of a2.themeFragmentsByKind)
        for (const [i, { styles: n }] of r2) {
          if (i === "default")
            continue;
          let o = n.cssText;
          a2.defaultFragments.has(i) || (o = o.replace(":host", `:host([${s}='${i}'])`)), t3.push(o);
        }
      window.ShadyCSS.ScopingShim.prepareAdoptedCssText(t3, this.localName), window.ShadyCSS.prepareTemplate(a2.template, this.localName);
    } else if (h) {
      const t3 = [];
      for (const s of e4)
        t3.push(s.styleSheet);
      this.shadowRoot.adoptedStyleSheets = t3;
    } else
      this.shadowRoot.querySelectorAll("style").forEach((s) => s.remove()), e4.forEach((s) => {
        const r2 = document.createElement("style");
        r2.textContent = s.cssText, this.shadowRoot.appendChild(r2);
      });
  }
  static registerThemeFragment(e4, t3, s) {
    const r2 = a2.themeFragmentsByKind.get(t3) || /* @__PURE__ */ new Map();
    r2.size === 0 && (a2.themeFragmentsByKind.set(t3, r2), r2.set("default", { name: e4, styles: s }), a2.defaultFragments.add(e4)), r2.set(e4, { name: e4, styles: s }), a2.instances.forEach((i) => i.shouldAdoptStyles());
  }
  _provideContext() {
    this._contextConsumers.forEach(([e4, t3]) => e4(this.lang, t3));
  }
  _handleContextPresence(e4) {
    e4.stopPropagation();
    const t3 = e4.composedPath()[0];
    if (this._contextConsumers.has(t3))
      return;
    this._contextConsumers.set(t3, [e4.detail.callback, () => this._contextConsumers.delete(t3)]);
    const [s, r2] = this._contextConsumers.get(t3) || [];
    s && r2 && s(this.lang || document.documentElement.lang || navigator.language, r2);
  }
};
a.themeFragmentsByKind = /* @__PURE__ */ new Map(), a.defaultFragments = /* @__PURE__ */ new Set(["spectrum"]), a.instances = /* @__PURE__ */ new Set();
var Theme = a;
...
Theme.registerThemeFragment("spectrum", "theme", theme_css_default);
...
`;
var scale_medium_css_default = e3;
